...
// Primary: gemini-2.5-flash  — best reasoning depth
// Fallback: gemini-2.5-flash-lite — stable, low-latency, less likely to 503
...
/**
 * Calls the Gemini generateContent API with automatic fallback.
 * Returns { result, modelUsed, usedFallback } so callers can surface the info.
 */
async function geminiWithFallback(
  params: { contents: string; config: object },
): Promise<{ result: any; modelUsed: string; usedFallback: boolean }> {
  ...
  for (const model of [GEMINI_PRIMARY, GEMINI_FALLBACK]) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        const usedFallback = model !== GEMINI_PRIMARY;
        if (usedFallback) {
          console.warn(`[gemini] ⚠️  Using fallback model ${model} — primary returned 503`);
        } else {
          console.log(`[gemini] ✓ model=${model}`);
        }
        return { result, modelUsed: model, usedFallback };
      } catch (e: any) {
        lastError = e;
        if (isOverloadError(e) && attempt < MAX_RETRIES - 1) {
          ...
          break; // non-503 error or out of retries for this model
        }
      }
    }
    if (!isOverloadError(lastError)) throw lastError; // bubble non-503 errors immediately
    console.warn(`[gemini] Model ${model} exhausted retries, trying next…`);
  }
...
async function callClarify(
  ...
  // Default: Gemini (with automatic fallback chain)
  const { result: response, modelUsed, usedFallback } = await geminiWithFallback({
    contents: `${CLARIFY_SYSTEM}\n\n${userPrompt}`,
    config: { responseMimeType: 'application/json', responseSchema: intentSchema },
  });
  const parsed = JSON.parse(response.text ?? '{}');
  if (usedFallback) parsed._fallback_notice = `Responded via fallback model (${modelUsed}) — primary was overloaded.`;
  return parsed;
}
...
async function callCompare(
  ...
  if (usedFallback) {
    console.warn(`[compare] ⚠️  Options generated by fallback model ${modelUsed}`);
    (parsed.options ?? []).forEach((o: any) => { o._fallback_notice = `Generated via fallback model (${modelUsed}).`; });
  }
  return parsed.options ?? [];
}
...
Output ONLY valid JSON: { "milestones": [ { id, title, description, depends_on } ] }`;
      ...
      if (provider === 'openai') {
        ...
        if (usedFallback) {
          console.warn(`[plan] ⚠️  Plan generated by fallback model ${modelUsed}`);
        }
        milestones = parsed.milestones ?? [];
      }
